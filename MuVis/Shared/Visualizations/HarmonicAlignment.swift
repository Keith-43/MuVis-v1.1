...
///
/// Created by Keith Bromley on 20 Nov 2020.   Significantly updated on17 Nov 2021.  Animated in Aug 2022.
...
struct HarmonicAlignment_Live: View {
    ...
    var body: some View {
        ...
        ForEach( 0 ..< harmonicCount, id: \.self) { harmonic in         //  0 <= harmonic < 6
            ...
            let hueIndex: Double = Double(harmonic) * harmOffset        // hueIndex = 0, 1/6, 2/6, 3/6, 4/6, 5/6
            
            ForEach( 0 ..< rowCount, id: \.self) { rowNum in            //  0 <= rowNum < 6
                
                HarmonicAlignment_Live_Shape(vector: muSpectrumA, har: harmonic, row: rowNum)
...
struct HarmonicAlignment_Live_Shape: Shape {
        ...
        for point in 0 ..< pointsPerOctave {
            ...
            */
            ...
            if(settings.optionOn == true) {
                harmAmp = (har == 0) ? 1.0 : Double(vector[row * pointsPerOctave + point])
            }
            else {
                harmAmp = 1.0
            }
            ...
            var magY: Double = Double(vector[cumulativePoints]) * rowHeight * harmAmp
