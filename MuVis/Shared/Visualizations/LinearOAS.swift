    ...
    var body: some View {
        ZStack {
            ...
            if(settings.optionOn) {
                GrayVertRectangles(columnCount: 96)         // struct code in VisUtilities file
...
struct LinearOAS_Live : View {
    ...
    var body: some View {
        ...
        ForEach( 0 ... 1, id: \.self) { rowNum in        //  rowNum = 0, 1
            LinearOAS_Live_Shape(vector: spectrumA, row: rowNum )
...
struct LinearOAS_Live_Shape: Shape {
    ...
    func path(in rect: CGRect) -> Path {
        ...
        // Render all 8 octaves in the bottom halfpane, and again in the upper halfplane:
        if (row == 0) {
        ...
        for oct in 0 ..< octaveCount {
            ...
            for bin in settings.octBottomBin[oct] ... settings.octTopBin[oct] {
                x = width * settings.binXFactor8[bin]
...
}  // end of LinearOAS_Live_Shape struct
