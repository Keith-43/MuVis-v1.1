...
///
/// Created by Keith Bromley on 29  Nov 2020.  Improved on 4 Jan 2022.  Animated in Aug 2022.
...
struct DoubleMuSpectrum_Live : View {
    ...
    var body: some View {
        ...
        DoubleMuSpectrum_Live_Shape(vector: muSpectrumA )
...
struct DoubleMuSpectrum_Live_Shape: Shape {
    ...
    public func path(in rect: CGRect) -> Path {
        ...
        for point in (1 ..< pointsPerRow).reversed()  {
            upRamp =  Double(point) / Double(pointsPerRow)   // upRamp goes from 0.0 to 1.0 as point goes from 0 to pointsPerRow
            x = upRamp * width
            magY = Double(vector[pointsPerRow + point]) * halfHeight
            magY = min(max(0.0, magY), halfHeight)
            y = halfHeight - magY
            path.addLine(to: CGPoint(x: x, y: y))
        }
