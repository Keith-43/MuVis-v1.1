...
///
/// Created by Keith Bromley on 21 Feb 2021 (from his previous java version for the Polaris app).   Animated in Aug 2022.
...
struct SpiralOAS_Live: View {
    ...
    var body: some View {
        ...
        ForEach( 0 ..< 8, id: \.self) { octave in        //  0 <= octave < 8

            SpiralOAS_Live_Shape(vector: spectrumA, oct: octave)
                ...
                .foregroundStyle( settings.optionOn ?
                    .angularGradient(settings.hueGradient,  center: UnitPoint(x: 0.5, y: 0.5), startAngle: Angle.degrees(-90.0), endAngle: Angle.degrees(270.0) ) :
                    .angularGradient( colors: [pomegranate, pomegranate],  center: UnitPoint(x: 0.5, y: 0.5), startAngle: Angle.degrees(-90.0), endAngle: Angle.degrees(270.0) ) )
        }
    }
}
...
struct SpiralOAS_Live_Shape: Shape {
...
}  // end of SpiralOAS_Live_Shape struct
