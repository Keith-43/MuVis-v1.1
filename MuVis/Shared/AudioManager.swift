...
/// The mixer node is used to convert the input audio stream to monophonic and convert the sampling rate to 44,100 sps (if necessary).
/// The delay node is used to introduce a delay into the audio stream (after our sampling tap) to synchronize the audio output with the on-screen rendered
...
class AudioManager: ObservableObject {
    ...
    // We will only consider the sixteen loudest peaks:
    static let peakCount: Int = 16
    ...
    // Requested size of the incoming buffers in sample frames (Supported range is [100, 400] ms):
    static let desiredBlockSampleCount: Int = 4410
    var actualBlockSampleCount: Int = 0    // will be set to the number of audio samples actually captured per block

    // one block of 4410 audio sample values:
    var blockSampleValues: [Float] = [Float](repeating: 0.0, count: desiredBlockSampleCount)

    static let circBufferLength: Int = 17640  // Must be >= 16384     I chose 4 * 4410 = 17,640
    // Store the most recent 17,640 samples in circBuffer.
    var circBuffer : [Float] = [Float] (repeating: 0.0, count: circBufferLength)

    var sampleValues: [Float] = [Float] (repeating: 0.0, count: AudioManager.fftLength)

    // Declare a circular buffer to store the past 48 blocks of the first six octaves of muSpectrum[].
    ...
    @Published var spectrum: [Float] = [Float](repeating: 0.0, count: binCount8)    // binCount8 = 3021+1 = 3022
    @Published var muSpectrum = [Float](repeating: 0.0, count: totalPointCount)     // totalPointCount = 96 * 12 = 1,152
    @Published var peakBinNumbers  = [Int]   (repeating: 0,   count: peakCount)
    @Published var peakAmps  = [Double](repeating: 0.0, count: peakCount)           // peak amplitudes

    // Declare a circular array to store the past 48 blocks of the first six octaves of muSpectrum[].
    // It stores 48*72*12=41,472 points
    ...
    func setupAudio(){
        ...
        player = AVAudioPlayerNode()        // player will read and play our song file  // needed here for Mic On/Mic Off to work properly in iOS
        ...
        do {
            ...
            // print("micFormat = \(micFormat)")  // shows that sampleRate = 48,000
            ...
            // Define a monophonic (1 ch) and 44100 sps AVAudioFormat for the desired output of our mixer node.
        ...
        // Even though I request 1024 samples, the app consistently gives me 4410 samples every 0.1 seconds.
        mixer.installTap(onBus:0,
                         bufferSize: AVAudioFrameCount(AudioManager.desiredBlockSampleCount),
                         format: nil) { (buffer, time) in
                        self.captureOutput(buffer: buffer)
            // This runs the capturOutput() func using the buffer created by the installTap() operation.
        ...
        delay.delayTime = 0.3   // The delay is specified in seconds. Default is 1. Valid range is 0 to 2 seconds.
        delay.feedback = 0.0    // Percentage of the output signal fed back. Default is 50%. Valid range is -100% to 100%.
        delay.lowPassCutoff = 5512  // The default value is 15000 Hz. The valid range is 10 Hz through (sampleRate/2).
        delay.wetDryMix = 100       // Blend is specified as a percentage. Default value is 100%.
                                    // Valid range is 0% (all dry) through 100% (all wet).
    ...
    func captureOutput(buffer: AVAudioPCMBuffer) {
        ...
        // var blockSampleValues: [Float] = [Float](repeating: 0.0, count: AudioManager.desiredBlockSampleCount)  // one block of 4410 audio sample values
        ...
        if(actualBlockSampleCount == AudioManager.desiredBlockSampleCount) {
    ...
    func processData(inputArray: [Float]) {
        ...
        // Apply the userGain to the blockSampleValues[] array:
        for sample in 0 ..< AudioManager.desiredBlockSampleCount {
            blockSampleValues[sample] = userGain * blockSampleValues[sample]
        }
        ...
        realOut.withUnsafeMutableBufferPointer { realOutBuffer in
            imagOut.withUnsafeMutableBufferPointer { imagOutBuffer in
                ...
                var binBuffer = [Float](repeating: 0.0, count: AudioManager.binCount8)
                ...
                let lowerBin: Int = settings.octBottomBin[0]    // lowerBin =  12
                let upperBin: Int = settings.octTopBin[5]       // upperBin = 755
                
                let result = peaksSorter.getSortedPeaks( binValues: binBuffer,
                                                         bottomBin: lowerBin,
                                                         topBin: upperBin,
                                                         peakThreshold: 0.1)
                ...
                //------------------------------------------------------------------------------------------------------
                // Finally, let's publish the results to the various Visualization Views:
                DispatchQueue.main.async { [self] in
                    spectrum        = binBuffer                 // binCount8 = 3021+1 = 3022
                    muSpectrum      = pointBuffer               // <- 96 * 12 = 1,152 points
                    muSpecHistory   = pointHistoryBuffer        // <- 48 * 72 * 12 = 48 * 864 = 41,472 points
                    peakBinNumbers  = sortedPeakBinNumbers      // peakCount = 16
                    peakAmps        = sortedPeakAmplitudes      // peakCount = 16
                }
            }
        }
